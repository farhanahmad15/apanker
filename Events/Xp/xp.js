  ...
  async execute(message) {
    ...
    Schema.findOne({ Guild: message?.guild.id }, async (err, data) => {
      ...
      if (Date.now() - user.timestamp > 60000) {
        const min = 15;
        const max = 30;
        const randomAmountOfXp = Math.floor(Math.random() * (max - min + 1)) + min;
        ...
        if (hasLeveledUp) {
          ...
          const sendEmbed = await message.guild.channels
            .cache.get(levelingChannel)
            .send(`**GG** ${message.author}, you just leveled up to level **${userData.level + 1}**!`);
      ...
      lock = false; // Release the lock after the save is complete
    });
  },
};
