    ...
    ticketSchema.findOne({ChannelID: channel.id}, async(err, data) =>{
        ...
        switch(customId){
            case "close":
                ...
                const deleted = await ticketSchema.deleteOne({ GuildID: guild.id, MemberID: member.id })
                ...
                .setDescription('Ticket will be closed in 10 seconds, enable DMS for the ticket transcript')
                .setColor('#CE3636')
                .setFooter({text: member.user.username, iconURL: member.displayAvatarURL({dynamic: true})})
                .setTimestamp()
                ...
                setTimeout(function(){
                    member.send({
                        embeds: [transcriptEmbed.setDescription(`Access your ticket transcript, copy this html and run this html code in a text editor or `)
                        .addFields(
                            {name: 'Here', value: 'https://www.w3schools.com/html/tryit.asp?filename=tryhtml_intro'}
                        )
                    ], 
                        files: [transcript]
                ...
                case "lock":
                    ...
                    channel.permissionOverwrites.edit(data.MemberID, {SendMessages: false})
                    ...
                    case "unlock":
                        ...
                        channel.permissionOverwrites.edit(data.MemberID, {SendMessages: true})
