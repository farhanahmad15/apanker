        ...
        switch (action) {
            case "add":
                ticketSchema.findOne({ GuildID: guildId, ChannelID: channel.id }, async (err, data) => {
                    if (err) throw err;
                    if (!data)
                        return interaction.reply({ embeds: [embed.setColor("#CE3636").setDescription("Something went wrong. Try again later.")], ephemeral: true });
                    ...
                    interaction.reply({ embeds: [embed.setColor("#36CE36").setDescription(`${member} has been added to the ticket.`)] });
            ...
            case "remove":
                ticketSchema.findOne({ GuildID: guildId, ChannelID: channel.id }, async (err, data) => {
                    ...
                    interaction.reply({ embeds: [embed.setColor("#36CE36").setDescription(`${member} has been removed from the ticket.`)] });
